...
class RegistrationAPIView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            message = EmailMessage('email/confirm_email.tpl', {'token': token}, 'from_email@gmail.com', to=[email])
...
class ActivationAPIView(APIView):
    ...
    def get(self, request, token, *args, **kwargs):
        ...
        if user_obj.is_active:
            return Response({"detail": "your account is activated"},status=status.HTTP_400_BAD_REQUEST)
        user_obj.is_active = True
        user_obj.save()
        return Response({"detail": "your account have been activated successfully"},status=status.HTTP_200_OK)
...
class ActivationResendAPIView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token = self.get_token_for_user(user_obj)
        message = EmailMessage('email/confirm_email.tpl', {'token': token}, 'from_email@gmail.com', to=[user_obj.email])
        EmailThread(message).start()
...
class PasswordResetRequestAPIView(generics.GenericAPIView):
    ...
    def post(self ,request ,*args ,**kwargs ):
        ...
        user_obj = serializer.validated_data['user']
        token = self.get_token_for_user(user_obj)
        message = EmailMessage('email/password_reset.tpl', {'token': token}, 'from_email@gmail.com', to=[user_obj.email])
        EmailThread(message).start()
        return Response({"details": "send test email"}, status= status.HTTP_200_OK)
